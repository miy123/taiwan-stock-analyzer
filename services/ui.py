...
def peer_pe_line(stock_id, pe, inline=False) -> str:
    """
    同業本益比對照（「同業中位 26.6 (+4%)」）—— 三頁共用的唯一實作。

    同業＝官方產業別分類，取該產業的本益比**中位數**
    （`services/sector.peer_pe`，資料來自已經在抓的兩份快照，不新增請求）。
    同業檔數不足就回空字串，寧可不顯示。

    ⚠️ 標籤一定要寫「中位」兩個字。使用者拿 8039 台虹去對照別的網站：
    本益比兩邊都是 81.59（同一個官方數字），但對方的「同業**平均**」是 46.18、
    我們的同業是 28.0，於是看起來像抓錯。實際上 46.18 可以完全重現
    ——那是「只算上市 84 檔的算術平均」，而我們是「上市+上櫃 162 檔的中位數」。
    電子零組件有霖宏 191.7／聯穎 189.6／凱崴 144.0 這種百倍股，
    平均 39.9 與中位 28.0 差了 12 倍本益比，正是不用平均的理由。
    光靠 tooltip 不夠——使用者要滑上去才看得到，那時他已經先困惑過一次了。
    ...
    if not p:
        return ""
    rel = p.get("rel_pct")
    body = (f"同業中位 {p['median']:.1f}"
            + (f" ({rel:+.0f}%)" if rel is not None else ""))
    # title 屬性不吃 markdown，不要在這裡寫 ** **
    tip = (f"{p['industry']} {p['n']} 檔（上市＋上櫃）的本益比中位數，不是平均"
           f"——本益比分布右偏，幾檔百倍股就能把平均拉高一截。"
           f"僅供定位參考，不進任何分數。")
