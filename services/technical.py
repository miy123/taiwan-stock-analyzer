...
def calculate_horizon_scores(df: pd.DataFrame) -> dict:
    ...
    if r120 is not None:
        if r120 > 60:
            # 這裡原本只給 +4（「漲幅已大」扣 6 分），實測方向是反的：
            # 139 期配對比較，同結構但半年漲 >60% 的那群（分數 88）3個月超額 +6.43%，
            # 漲 10~60% 的那群（分數 94）只有 +2.07%，差 4.36%、t=-3.33。
            # 改回 +10 後，選前10名的 3個月超額 +3.77% → +6.70%（t=2.55），
            # 且走查前後半段皆為正。與本專案「動能在各種環境都有效」的結論一致。
            # ⚠️ 代價：漲多那群報酬高但勝率低（61% vs 70%），組合會更顛簸。
            #    「漲多」的風險改由 overheat_flag() 的警語提醒，不再用扣分處理——
            #    扣分會讓它排不進前段，等於直接放棄報酬最高的一群。
            lg += 10; lgd.append(f"半年漲 {r120:.0f}%，強勢動能延續 (+10)")
        elif r120 > 10:
