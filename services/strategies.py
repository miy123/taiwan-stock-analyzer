...
#
#   `strategy_comparison.py` 回測時每一列的基本面都無法還原——財報沒有
#   point-in-time 快照（納入會造成前視偏誤），所以任何以基本面為條件的策略
#   都**量不出成績**。做成策略只會讓實證面板印出看起來很權威的假數字。
#
# 做成加掛條件，責任就清楚了：排序仍由有回測背書的訊號負責，
# 體質門檻是使用者自己疊上去的風險偏好，畫面必須講明它沒有實證支持。
#
# ⚠️ 用的是 **fund_score（基本面分）**，不是綜合評分。綜合評分有 35% 是技術分
#    （與趨勢結構分重複計算同一件事）、15% 新聞、15% 分析師目標價，
#    拿它當「體質」名實不符——使用者要的是「直接從營收之類的基本面看」。
